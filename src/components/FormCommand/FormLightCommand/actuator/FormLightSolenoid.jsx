...
const FormLightSolenoid = () => {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .post(`${process.env.REACT_APP_API}/createLightSolenoid`, { actuatorName, lightValue, status, condition, commandId })
      .then((response) => {
  ...
  const addId = () => {
    document.getElementById("commandIdLightSolenoid").value = crypto.randomUUID();
  };

  return (
    <div>
      <div className="container" id="condition-light-actuator-display4">
        <div className="container text-center display-block main-block">
          ...
          <form onSubmit={submitForm}>
            <div className="row align-items-start command-form">
              ...
              <div className="col column-line command-actuator">
                <button id="commandIdLightSolenoid" type="submit" value="" onMouseMove={addId} onClick={inputValue("commandId")} className="form-submit">
                  Save
                </button>
              </div>
            </div>
...
export default FormLightSolenoid;
