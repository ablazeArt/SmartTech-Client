...
const FormTempSolenoid = () => {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .post(`${process.env.REACT_APP_API}/createTempSolenoid`, { actuatorName, tempValue, status, condition, commandId })
      .then((response) => {
  ...
  const addId = () => {
    document.getElementById("commandIdTempSolenoid").value = crypto.randomUUID();
  };

  return (
    <div>
      <div className="container" id="condition-temp-actuator-display4">
          ...
          <form onSubmit={submitForm}>
            <div className="row align-items-start command-form">
              ...
              <div className="col column-line command-actuator">
                ...
                <button id="commandIdTempSolenoid" type="submit" value="" onMouseMove={addId} onClick={inputValue("commandId")} className="form-submit">
                  Save
                </button>
              </div>
            </div>
...
export default FormTempSolenoid;
