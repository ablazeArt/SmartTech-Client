...
const FormLevelLed = () => {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .post(`${process.env.REACT_APP_API}/createLevelLed`, { actuatorName, levelValue, status, condition, commandId })
      .then((response) => {
  ...
  const addId = () => {
    document.getElementById("commandIdLevelLed").value = crypto.randomUUID();
  };

  return (
    <div>
      <div className="container" id="condition-level-actuator-display3">
        <div className="container text-center display-block main-block">
          ...
          <form onSubmit={submitForm}>
            <div className="row align-items-start command-form">
              ...
              <div className="col column-line command-actuator">
                <button id="commandIdLevelLed" type="submit" value="" onMouseMove={addId} onClick={inputValue("commandId")} className="form-submit">
                  Save
                </button>
              </div>
            </div>
