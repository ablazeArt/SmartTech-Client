...
const ManualLedCommand = (props) => {
  const [commands, setCommands] = useState([]);
  const [actuatorStatus, setActuatorStatus] = useState([]);
  const fetchData = () => {
    axios
      .get(`${process.env.REACT_APP_API}/commandsManualLed`)
      .then((response) => {
  ...
  const deleteCommmand = (id) => {
    ...
    axios
      .delete(`${process.env.REACT_APP_API}/commandManualLed/${id}`)
      .then((response) => {
        Swal.fire({
...
export default ManualLedCommand;
