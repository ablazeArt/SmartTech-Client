...
const FormManualLed = () => {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .post(`${process.env.REACT_APP_API}/createManualLed`, { actuatorName, status, commandId })
      .then((response) => {
  ...
  const addId = () => {
    document.getElementById("commandIdManualLed").value = crypto.randomUUID();
  };

  return (
    <div>
      <div className="container" id="manual-actuator-display3">
        <div className="container text-center display-block main-block">
          ...
          <form onSubmit={submitForm}>
            <div className="row align-items-start command-form">
              ...
              <div className="col column-line command-actuator">
                <button id="commandIdManualLed" type="submit" value="" onMouseMove={addId} onClick={inputValue("commandId")} className="form-submit">
                  Save
                </button>
              </div>
            </div>
