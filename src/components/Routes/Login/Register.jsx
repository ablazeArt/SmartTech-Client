...
const Register = () => {
  ...
  const register = () => {
    ...
    if (name && email && password) {
      axios.post(`${process.env.REACT_APP_API}/Register`, user).then((res) => console.log(res));
    } else {
      alert("Invalid input");
    }
  };
  ...
  return (
    <div class="flex flex-col max-w-md px-4 py-8 bg-white rounded-lg shadow dark:bg-gray-800 sm:px-6 md:px-8 lg:px-10">
      <div class="self-center mb-2 text-xl font-light text-gray-800 sm:text-2xl dark:text-white">Create a new account</div>
