...
const TempDcEdit = () => {
  ...
  const { actuatorName, tempValue, direction, speed, condition } = state;
  const { commandId } = useParams();

  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API}/commandTempDc/${commandId}`)
      .then((response) => {
        //แยกข้อมูล
        const { tempValue, direction, speed, condition } = response.data;
        //เก็บข้อมูลเดิม
        setState({ ...state, tempValue, direction, speed, condition });
      })
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .put(`${process.env.REACT_APP_API}/commandTempDc/${commandId}`, {
        actuatorName,
        tempValue,
        speed,
        direction,
