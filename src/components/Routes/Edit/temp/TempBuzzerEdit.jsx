...
const TempBuzzerEdit = () => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API}/commandTempBuzzer/${commandId}`)
      .then((response) => {
        //แยกข้อมูล
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .put(`${process.env.REACT_APP_API}/commandTempBuzzer/${commandId}`, {
        actuatorName,
        tempValue,
        status,
        condition
