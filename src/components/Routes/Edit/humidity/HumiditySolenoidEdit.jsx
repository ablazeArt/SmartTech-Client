...
const HumiditySolenoidEdit = () => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API}/commandHumiditySolenoid/${commandId}`)
      .then((response) => {
        //แยกข้อมูล
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .put(`${process.env.REACT_APP_API}/commandHumiditySolenoid/${commandId}`, {
        actuatorName,
        humidityValue,
        status,
        condition
...
export default HumiditySolenoidEdit;
