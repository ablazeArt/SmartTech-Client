...
const LightDcEdit = () => {
  ...
  const { actuatorName, lightValue, direction, speed, condition } = state;
  const { commandId } = useParams();

  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API}/commandLightDc/${commandId}`)
      .then((response) => {
        //แยกข้อมูล
        const { lightValue, direction, speed, condition } = response.data;
        //เก็บข้อมูลเดิม
        setState({ ...state, lightValue, direction, speed, condition });
      })
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .put(`${process.env.REACT_APP_API}/commandLightDc/${commandId}`, {
        actuatorName,
        lightValue,
        speed,
        direction,
