...
const LevelDcEdit = () => {
  ...
  const { actuatorName, levelValue, direction, speed, condition } = state;
  const { commandId } = useParams();

  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API}/commandLevelDc/${commandId}`)
      .then((response) => {
        //แยกข้อมูล
        const { levelValue, direction, speed, condition } = response.data;
        //เก็บข้อมูลเดิม
        setState({ ...state, levelValue, direction, speed, condition });
      })
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .put(`${process.env.REACT_APP_API}/commandLevelDc/${commandId}`, {
        actuatorName,
        levelValue,
        speed,
        direction,
