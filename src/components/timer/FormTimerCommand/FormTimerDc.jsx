...
const FormTimerDc = () => {
  ...
  const { actuatorName, dayStart, hourStart, minuteStart, dayEnd, hourEnd, minuteEnd, speed, direction, commandId } = state;
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .post(`${process.env.REACT_APP_API}/createTimerDc`, { actuatorName, dayStart, hourStart, minuteStart, dayEnd, hourEnd, minuteEnd, speed, direction, commandId })
      .then((response) => {
  ...
  const addId = () => {
    document.getElementById("commandIdTimerDc").value = crypto.randomUUID();
  };

  return (
    <div>
      <div className="container" id="timer-actuator-display2">
          ...
          <form onSubmit={submitForm}>
            <div className="row align-items-start command-form">
              ...
              <div className="col column-line command-actuator">
                ...
                <div>
                  <button id="commandIdTimerDc" type="submit" value="" onMouseMove={addId} onClick={inputValue("commandId")} className="form-submit">
                    Save
                  </button>
                </div>
              </div>
