...
const FormTimerSolenoid = () => {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    axios
      .post(`${process.env.REACT_APP_API}/createTimerSolenoid`, { actuatorName, dayStart, hourStart, minuteStart, dayEnd, hourEnd, minuteEnd, status, commandId })
      .then((response) => {
  ...
  const addId = () => {
    document.getElementById("commandIdTimerSolenoid").value = crypto.randomUUID();
  };

  return (
    <div>
      <div className="container" id="timer-actuator-display4">
        <div className="container text-center display-block main-block">
          ...
          <form onSubmit={submitForm}>
            <div className="row align-items-start command-form">
              ...
              <div className="col column-line command-actuator">
                <button id="commandIdTimerSolenoid" type="submit" value="" onMouseMove={addId} onClick={inputValue("commandId")} className="form-submit">
                  Save
                </button>
              </div>
            </div>
...
export default FormTimerSolenoid;
