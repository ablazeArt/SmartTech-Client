...
const IrsensorBuzzer = (props) => {
  const [commands, setCommands] = useState([]);
  const [actuatorStatus, setActuatorStatus] = useState([]);
  const fetchData = () => {
    axios
      .get(`${process.env.REACT_APP_API}/commandsIrsensorBuzzer`)
      .then((response) => {
  ...
  const deleteCommmand = (id) => {
    ...
    axios
      .delete(`${process.env.REACT_APP_API}/commandIrsensorBuzzer/${id}`)
      .then((response) => {
        Swal.fire({
    ...
    <div>
      {commands.map((command, index) => (
        <div className="col-12 column-line list-display">
          ...
          <p className="list-display-text">Initail Irsensor:{command.irsensorValue}</p>
          <p className="list-display-text">Status:{command.status}</p>
          <button>
            <Link to={`commandIrsensorBuzzer/edit/${command.commandId}`} className="list-display-text edit-btn">
              Edit
            </Link>
          </button>
          ...
          {actuatorStatus.map((actuatorStatus, index) => (
            <div>
              <img className="img-status" src={actuatorStatus.buzzer === "online" ? require("../images/green-icon.png") : require("../images/red-icon.png")} alt="" />
            </div>
          ))}
        </div>
      ))}
