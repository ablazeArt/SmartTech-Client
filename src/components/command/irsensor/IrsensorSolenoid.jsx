...
const IrsensorSolenoid = (props) => {
  const [commands, setCommands] = useState([]);
  const [actuatorStatus, setActuatorStatus] = useState([]);
  const fetchData = () => {
    axios
      .get(`${process.env.REACT_APP_API}/commandsIrsensorSolenoid`)
      .then((response) => {
  ...
  const deleteCommmand = (id) => {
    ...
    axios
      .delete(`${process.env.REACT_APP_API}/commandIrsensorSolenoid/${id}`)
      .then((response) => {
        Swal.fire({
    ...
    <div>
      {commands.map((command, index) => (
        <div className="col-12 column-line list-display">
          ...
          <button>
            <Link to={`commandIrsensorSolenoid/edit/${command.commandId}`} className="list-display-text edit-btn">
              Edit
            </Link>
          </button>
...
export default IrsensorSolenoid;
