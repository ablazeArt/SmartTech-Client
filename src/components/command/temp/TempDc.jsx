...
const TempDc = (props) => {
  ...
  const fetchData = () => {
    axios
      .get(`${process.env.REACT_APP_API}/commandsTempDc`)
      .then((response) => {
  ...
  const deleteCommmand = (id) => {
    ...
    axios
      .delete(`${process.env.REACT_APP_API}/commandTempDc/${id}`)
      .then((response) => {
        Swal.fire({
    ...
    <div>
      {commands.map((command, index) => (
        <div className="col-12 column-line list-display">
          ...
          <button>
            <Link to={`commandTempDc/edit/${command.commandId}`} className="list-display-text edit-btn">
              Edit
            </Link>
          </button>
