...
const TempBuzzer = (props) => {
  const [commands, setCommands] = useState([]);
  const [actuatorStatus, setActuatorStatus] = useState([]);
  const fetchData = () => {
    axios
      .get(`${process.env.REACT_APP_API}/commandsTempBuzzer`)
      .then((response) => {
  ...
  const deleteCommmand = (id) => {
    ...
    axios
      .delete(`${process.env.REACT_APP_API}/commandTempBuzzer/${id}`)
      .then((response) => {
        Swal.fire({
    ...
    <div>
      {commands.map((command, index) => (
        <div className="col-12 column-line list-display">
          ...
          <p className="list-display-text">Initail Temperature:{command.tempValue}</p>
          <p className="list-display-text">Status:{command.status}</p>
          <button>
            <Link to={`commandTempBuzzer/edit/${command.commandId}`} className="list-display-text edit-btn">
              Edit
            </Link>
          </button>
