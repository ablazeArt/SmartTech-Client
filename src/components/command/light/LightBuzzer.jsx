...
const LightBuzzer = (props) => {
  const [commands, setCommands] = useState([]);
  const [actuatorStatus, setActuatorStatus] = useState([]);
  const fetchData = () => {
    axios
      .get(`${process.env.REACT_APP_API}/commandsLightBuzzer`)
      .then((response) => {
  ...
  const deleteCommmand = (id) => {
    ...
    axios
      .delete(`${process.env.REACT_APP_API}/commandLightBuzzer/${id}`)
      .then((response) => {
        Swal.fire({
    ...
    <div>
      {commands.map((command, index) => (
        <div className="col-12 column-line list-display">
          ...
          <p className="list-display-text">Initail Light:{command.lightValue}</p>
          <p className="list-display-text">Status:{command.status}</p>
          <button>
            <Link to={`commandLightBuzzer/edit/${command.commandId}`} className="list-display-text edit-btn">
              Edit
            </Link>
          </button>
